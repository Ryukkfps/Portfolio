    ...
    <div className="bg-[#111111] min-h-screen pb-24 animate-fadeIn">
      ...
      <div className="py-24 px-8 border-b border-white/5">
        <div className="max-w-7xl mx-auto">
          <p className="text-[#a3e635] text-[10px] font-black tracking-[0.4em] uppercase mb-4">Capability</p>
      ...
      <div className="max-w-7xl mx-auto px-8 mt-24 space-y-32">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-1px bg-white/10 border border-white/10">
          {services.map((service, index) => (
            <div key={index} className="bg-[#111111] p-12 hover:bg-[#a3e635]/5 transition-all group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16">
            {categories.map((category) => (
              <div key={category} className="space-y-8">
                <h3 className="text-[#a3e635] text-[10px] font-black tracking-[0.4em] uppercase border-b border-white/10 pb-4">
                  {category}
                </h3>
                <div className="flex flex-wrap gap-3">
                  {skills.filter(s => s.category === category).map((skill) => (
                    <div 
                      key={skill.id} 
                      className="px-4 py-2 bg-white/5 border border-white/10 text-white font-bold uppercase tracking-widest text-[10px] hover:border-[#a3e635] hover:text-[#a3e635] transition-all cursor-default"
                    >
                      {skill.name}
                    </div>
                  ))}
        ...
        {/* CTA */}
        <div className="bg-[#a3e635] p-16 text-center">
          <h2 className="text-4xl md:text-6xl font-black text-black uppercase tracking-tighter italic mb-8">
            Let's build something<br/>extraordinary.
          </h2>
          <Link
            href="/contact"
            className="inline-block bg-black text-[#a3e635] px-12 py-5 text-[10px] font-black tracking-[0.4em] hover:bg-white hover:text-black transition-all"
          >
